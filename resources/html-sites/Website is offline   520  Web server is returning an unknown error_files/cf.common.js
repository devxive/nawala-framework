...
(function($, Polyglot, undefined){
	var polyglot = new Polyglot();

	// Get a cookie value
	function getCookie( name ){
		var cookie = document.cookie.match('(^|;) ?' + name + '=([^;]*)(;|$)');
		return cookie && cookie[2] ? unescape(cookie[2]) : false;
	};

	// Check if a cookie exists. Used for testing cookie support
	...
	$(function(){
		...
		// If this cookie isn't found, assume they don't have cookies enabled
		// and show a banner
		if( !cookieExists('__cfduid') ){
			$('#cookie-alert').show();
		}
	});

})( window.$, window.Polyglot, window );
