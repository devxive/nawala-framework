 ...
 * @since          1.5
 */
class NFWInstaller extends JInstaller
{
	...
	public function setAdapter($name, &$adapter = null, $options = Array())
	{
		...
		$class    = "NFWInstaller" . ucfirst($name);
		if (!class_exists($class)) {
			if (!is_object($adapter)) {
				foreach ($adapters as $nfw_adapter) {
					// Try to load the adapter object
					$fullpath = dirname(__FILE__) . '/' . 'adapters' . '/' . strtolower($nfw_adapter) . '.php';
